...
public class BiDirectionTests {
    ...
    /* 필기.
    *   데이터베이스의 테이블은 오래키 하나로 양방향 조회가 가능하지만
    ...
    *   양방향 연관관계 매핑 시 연관관게의 주인은 비즈니스 로직 상 중요하다고 해서
    *   주인이 되는 것이 아니다.
    *   그러면 누가 주인이 될 것인가?
    ...
    * */
    ...
    @DisplayName("양방향 연관관계 매핑 조회 테스트1(연관관계의 주인이 아닌 객체)")
    @Test
    void notOwnerFindTest2(){
        ...
        //when
        /* 해당 엔티티를 조회하고 필요 시 연관관계 엔티팉를 조회하는 쿼리를 실행한다. */
    ...
    // StackOverflowError -> 양방향이 이기때문에 양쪽에서 불러와서(너무 많이 가져옴) 터져버림
    // Category class 에서        ", menuList=" + menuList  때문에 계속 가지고옴// @To
