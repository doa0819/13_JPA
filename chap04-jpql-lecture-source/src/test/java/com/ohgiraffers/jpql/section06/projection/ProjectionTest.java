...
public class ProjectionTest {
    ...
    /* 필기.
    *   [프로젝션(Projection)]
    *   select 절 조회할 대상 지정하는 것을 프로젝션이라고 한다.
    *   (SELECT {프로젝션 대상} FROM ...)
    * */
    ...
    *       - 원하는 객체를바로 조회할 수 있다.
    *       - 조회 된 엔티티는 영속성 컨텍스트(앤티티매니저가 관리하는 인스턴트들의 집합 -> 데이터베이스랑 동기화)가 관리한다.
    *   2. 임베디드 타입 프로젝션
    *       - @Embeddable -> 엔티티와 거의 비슷하게 사용되며 (엔티티는 아니다!!) 조회의 시작점이 될 수 없다(from 절에 사용 불가)
    *       - 임베디드 타입은 영속 컨텍스트에서 관리하지 않는다.
    *   3. 스칼라 타입 프로젝션
    *       - 숫자, 문자, 날짜 등등의 기본 데이터 타입을 의미한다. (예) List(String))
    *       - 스칼라 타입도 영속성 컨텍스트에서 관리되지 않는다.
    *   4. new 명령어를 활용한 프로젝션
    *       - 다양한 종류의 단순 값들을 DTO 로 바로 조회하는 방식으로 'new 패키지명.DTO명' 을 쓰게 되면
    *         역시 이 친구도 클래스 객체는 멘티티가 아니기 때문에 영속성 컨텍스트에서 관리되지 않는다.
    *
    * */
