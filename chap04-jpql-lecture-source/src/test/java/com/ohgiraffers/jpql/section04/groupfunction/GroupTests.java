...
public class GroupTests {
    ...
    /*     카운드, 합계 / 그룹함수 해빙 서브쿼리 -> 혹시 모를 상황 대비 => 관리자가 사용 */
    ...
    *   2. 값이 없는 상태에서 count 를 제외한 그룹 함수는 null 이 되고, count만 0이 된다.
    ...
    *       그룹 함수 결과 값과 비교하기 위한 파라미터 타입은 Long 또는 Double 로 해야 한다.
    * */

    /* 필기.
    *   Group By -> 데이터를 특정 컬럼을 기준으로 그룹화 한다.
    *   select category_code, count(*) from tbl_menu 카테고리를 기준으로 그룹화 한다.
    *   Having 절은 Group By 절로 그룹화 된 결과에 조건을 지정 -> where
    ...
    * */
    ...
    void testCountMenuOfCategory(){
        ...
        // categoryCode = 4 가 몇개 있는지 세고 싶은때
        int categoryCode = 4;
