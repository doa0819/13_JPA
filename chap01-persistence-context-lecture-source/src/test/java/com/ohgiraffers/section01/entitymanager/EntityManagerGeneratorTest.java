...
public class EntityManagerGeneratorTest {
    ...
    *   엔티티 매니저 팩토리는 tread-safe(여러사람들이 한번에 접근해도 데이터의 변형이 없다) 하기 때문에 여러 쓰레드가
    *   동시에 접근해도 안전하기 때문에 공유해서 재사용을 한다.
    *   -> 그래서 싱글톤으로 관리를 한다.
    *   tread-safe(여러사람들이 접근해도 안전) 한 기능들은 매번 생성하기 에는 비용(시간, 메모리)
    *   부담이 크기 때문에 application 스코프과 동일하게
    *   singleton 으로 생성해서 관리하는 것이 효율적이다.
    *   따라서 데이터베이스를 사용하는 어플리케이션 당 한 개의
    *   EntityManagerFactory 를 생성한다.
    *    -> 팩토리가 붙은 애들은 싱글톤으로 관리 한다. (예) 한명의 매니저가 하나의 엔티티에 붙는다.)
    *    -> 하나의 시작부터 끝까지 같이 간다 (하나당 하나만 만들기 때문에 안정성이 있다 그러나 메모리는 커진다 왜냐 하나당 하나의 공간만 만들기 때문에)
    * */
    ...
    *   따라서 web 의 경우 일반적으로 request-scope (하나의 기능을 요청하고 그 기능이 끝날때 까지)와 일치 시킨다.
    *   request-scope -> 부를때 마다 하나씩 생성해서 관리
    * */
